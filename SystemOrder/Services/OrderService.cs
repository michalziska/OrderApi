...
namespace SystemOrder.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<PagedList<Product>> ListProductsAsync(ProductResourceParameters productResourceParameters)
        {
            try
            {
                var query = await _productRepository.ListAsync(productResourceParameters);

                return query;
            }
            catch (Exception ex)
            {
                return null; //new OrderResponse($"An error occurred when ...: {ex.Message}");
            }
        }
